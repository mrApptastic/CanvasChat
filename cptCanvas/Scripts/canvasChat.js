...
var methods = ["Star", "Circle", "FillRect", "StrokeRect", "Rect"];

$(function () {
    var myName = "Hans Hansen";
    var myToast = "success";
    var elem = document.getElementById("canvasBandit");
    var ovel = document.getElementById("overBandit");
    elem.width = window.innerWidth;
    elem.height = window.innerHeight;
    ovel.width = window.innerWidth;
    ovel.height = window.innerHeight;

    var elme = elem.getContext("2d");
    var over = ovel.getContext("2d");

    var fill = "Black";
    var stro = "Transparent";
    var meth = "Circle";
    var width = 5;
    var height = 5;
    var line = 1;
    var shCo = "Transparent";
    var shBl = 0;
    var shOx = 0;
    var shOy = 0;
    ...
    function draw(sx, sy, w, h, f, s, l, m, o, u, z, q, overlay) {
        var ctx = overlay === true ? over  : elme;
        ...
        var offX = elem.getBoundingClientRect().left + window.scrollX;
        var offY = elem.getBoundingClientRect().top + window.scrollY;
    ...
    function clearCanvas(overlay) {
        var ctx = overlay === true ? ovel.getContext("2d") : elem.getContext("2d");
    ...
    chat.client.receiveCoordinates = function (x, y, w, h, f, s, l, m, o, u, z, q) {
        draw(x, y, w, h, f, s, l, m, o, u, z, q);
    };
    ...
    $.connection.hub.start().done(function () {
        ...
        ovel.addEventListener("mousemove", function (event) {
            var x = event.pageX;
            var y = event.pageY;
            if (event.buttons && event.buttons > 0) {
                ...
                draw(x, y, width, height, fill, stro, line, meth, shCo, shBl, shOx, shOy, true);
            }
        });

        ovel.addEventListener("mousedown", function (event) {
            var x = event.pageX;
            var y = event.pageY;
        ...
        document.getElementById("heightRange").addEventListener("change", function (event) {
            height = this.value;
        });
        document.getElementById("widthRange").addEventListener("change", function (event) {
            width = this.value;
        });
        document.getElementById("lineRange").addEventListener("change", function (event) {
            line = this.value;
        });
        document.getElementById("shadeBlurRange").addEventListener("change", function (event) {
            shBl = this.value;
        });
        document.getElementById("shadeXRange").addEventListener("change", function (event) {
            shOx = this.value;
        });
        document.getElementById("shadeYRange").addEventListener("change", function (event) {
            shOy = this.value;
        });        

        var met = methods.sort();

        var methodSelect = document.getElementById("methodSelect");

        for (let i = 0; i < met.length; i++) {
            methodSelect.innerHTML += "<option value='" + met[i] + "'>" + met[i] + "</option>";
        }

        methodSelect.getElementsByTagName("option")[met.indexOf(meth)].setAttribute("selected", "");

        methodSelect.addEventListener("change", function (event) {
            meth = this.value;
        });

        var col = colours.sort();

        var colourSelect = document.getElementById("colourSelect");
        var strokeSelect = document.getElementById("strokeSelect");
        var shadeSelect = document.getElementById("shadeSelect");

        for (let i = 0; i < col.length; i++) {
            colourSelect.innerHTML += "<option style='background: " + col[i] + " ;' value='" + col[i] + "'>" + col[i] + "</option>";
            strokeSelect.innerHTML += "<option style='background: " + col[i] + " ;' value='" + col[i] + "'>" + col[i] + "</option>";
            shadeSelect.innerHTML += "<option style='background: " + col[i] + " ;' value='" + col[i] + "'>" + col[i] + "</option>";
        }

        colourSelect.getElementsByTagName("option")[col.indexOf(fill)].setAttribute("selected", "");
        strokeSelect.getElementsByTagName("option")[col.indexOf(stro)].setAttribute("selected", "");
        shadeSelect.getElementsByTagName("option")[col.indexOf(shCo)].setAttribute("selected", "");
        ...
        shadeSelect.addEventListener("change", function (event) {
            shCo = this.value;
        });

        document.getElementById("message").addEventListener("keydown", function (event) {
            if (event.keyCode === 13) {
                chat.server.send(myName, event.target.value, myToast);
                event.target.value = "";
            }            
        });
